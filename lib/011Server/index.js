  ...
  innerComunication({ sourceType, socket, session, event, data, req, res, next }) {
    ...
    case 'rest':
      ...
      request = {
        ...
        getData: function () {
          let sources = [];
          for (let p in this.data) sources.push(this.data[p]);
  ...
  outerComunication(req) {
    ...
    return (data, type ='json') => {
      ...
      case 'socket':
        switch (type) {
        case 'json':
          return req.socket.sio.to(req.socket.socket.id).emit(req.event, data);
        default:
          // nothig
        }
        break;
    ...
    if (this.n().config.socketio) {
      this.sio.on("connection", (socket) => {
        ...
        socket.onevent = function(packet) {
          ...
          this.emit.apply   (this, ["*"].concat(args)); // additional call to catch-all
        };
        socket.on("*", (e, data) => {
